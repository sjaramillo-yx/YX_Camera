...
esp_err_t ethman_init(esp_eth_handle_t *eth_handle_out) {
  ...
  /* Initialize Ethernet */
  ESP_GOTO_ON_FALSE(eth_handle_out != NULL, ESP_ERR_INVALID_ARG,
                    err, TAG, "invalid arguments: ethernet handle is already initialized!");
  ...
  ESP_GOTO_ON_FALSE(esp_netif_init() == ESP_OK, ESP_FAIL,
                    err, TAG, "Couldn't initialize netif!");
  ret = esp_event_loop_create_default();
  ESP_GOTO_ON_FALSE(ret == ESP_OK, ret,
                    err, TAG, "Couldn't create default event loop!");
  ...
  ESP_GOTO_ON_FALSE(ret == ESP_OK, ret,
                    err, TAG, "Couldn't attach netif!");
  /* Start the Ethernet state machine*/
  ret = esp_eth_start(eth_handle);
  ESP_GOTO_ON_FALSE(ret == ESP_OK, ret,
                    err, TAG, "Couldn't start the Ethernate state machine!");
  /* Output and return */
