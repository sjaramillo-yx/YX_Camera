...
# Files
CREDS_FILE="${SCRIPT_DIR}/${BOOTSTRAP_USER}Keys.json"
...
if aws iam get-user --user-name "${BOOTSTRAP_USER}" >/dev/null 2>&1; then
  echo "User '${BOOTSTRAP_USER}' already exists, deleting it..."

  # Detach managed policies
  for arn in $(aws iam list-attached-user-policies  --user-name "${BOOTSTRAP_USER}" \
                                                    --query 'AttachedPolicies[].PolicyArn' \
                                                    --output text); do
    aws iam detach-user-policy --user-name "${BOOTSTRAP_USER}" --policy-arn "${arn}"
  done

  # Delete inline policies
  for policy in $(aws iam list-user-policies  --user-name "${BOOTSTRAP_USER}" \
                                              --query 'PolicyNames[]' \
                                              --output text); do
    aws iam delete-user-policy --user-name "${BOOTSTRAP_USER}" --policy-name "${policy}"
  done

  # Delete access keys
  for key in $(aws iam list-access-keys --user-name "${BOOTSTRAP_USER}" \
                                        --query 'AccessKeyMetadata[].AccessKeyId' \
                                        --output text); do
    aws iam delete-access-key --user-name "${BOOTSTRAP_USER}" --access-key-id "${key}"
  done

  # Finally delete the user
  aws iam delete-user --user-name "${BOOTSTRAP_USER}"
  echo "User '${BOOTSTRAP_USER}' deleted"
fi
...
# --------
aws iam attach-user-policy  --user-name "${BOOTSTRAP_USER}" \
                            --policy-arn "${POLICY_ARN}"

# --------
# 4) Create access keys for the bootstrap user (SAVE THE SECRET ONCE)
# --------
aws iam create-access-key --user-name "${BOOTSTRAP_USER}" --output json > "${CREDS_FILE}"
...
aws configure set aws_access_key_id     "${ACCESS_KEY_ID}"     --profile "${BOOTSTRAP_USER}"
aws configure set aws_secret_access_key "${SECRET_ACCESS_KEY}" --profile "${BOOTSTRAP_USER}"
aws configure set region                "${AWS_REGION}"        --profile "${BOOTSTRAP_USER}"

echo "Validating bootstrapper profile '${BOOTSTRAP_USER}'..."
aws sts get-caller-identity --profile "${BOOTSTRAP_USER}" --region "${AWS_REGION}" 
